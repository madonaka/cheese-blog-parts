...
while(off+8<=shp.length){
  ...
  if(type===3||type===13){
    ...
    if(!(xmax<X0||xmin>X1||ymax<Y0||ymin>Y1)){
      ...
      const inK=!(xmax<KBOX[0]||xmin>KBOX[2]||ymax<KBOX[1]||ymin>KBOX[3]) || !(xmax<JBOX[0]||xmin>JBOX[2]||ymax<JBOX[1]||ymin>JBOX[3]);
...
fs.writeFileSync("hydro-raw4.json", JSON.stringify(kept));
console.log("hydro-raw4.json", Math.round(fs.statSync("hydro-raw4.json").size/1048576)+"MB");
